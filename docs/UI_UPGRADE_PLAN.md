...
- Interface-Ausbaustufe 1 ist umgesetzt: automatischer `Client/resmenu`-Katalog, Suche, TGA/DDS- sowie PNG/JPG/BMP-Vorschau (WIC unter Windows) und read-only NIF-/Materialanalyse für UI-NIFs. Schreibende Interface-Bearbeitung, Drop-Table-Editor und schreibendes NIF-Material-Editing bleiben offen; AI Workspace ist umgesetzt. Der Drop-Table-Editor bleibt zurückgestellt, bis die bekannten ShineText-Kanten der 270+ Spalten breiten `ItemDropTable.txt` belastbar behoben sind.
