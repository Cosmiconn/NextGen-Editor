...
- Interface-Ausbaustufe 1 ist umgesetzt: automatischer `Client/resmenu`-Katalog, Suche, TGA/DDS- sowie PNG/JPG/BMP-Vorschau (WIC unter Windows) und read-only NIF-/Materialanalyse für UI-NIFs. Als erste schreibende, formatsichere Stufe gibt es jetzt non-destruktive Projekt-Overrides: ein ausgewähltes Asset wird unverändert nach `<Projekt>/Client/resmenu/...` kopiert, die Vorschau bevorzugt diese Projektkopie und der Override kann wieder entfernt werden, ohne den Clientbestand anzutasten. Inhaltliche Interface-Bearbeitung und schreibendes NIF-Material-Editing bleiben offen; AI Workspace ist umgesetzt.
