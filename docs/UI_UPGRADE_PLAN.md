...
- Interface Editor: read-only `resmenu`-Browser mit TGA/DDS/PNG/JPG/BMP-Vorschau und UI-NIF-/Materialinspektor ist umgesetzt; schreibende Layout-/Asset-Bearbeitung bleibt offen.
- Drop Table Editor.
...
- Interface-Ausbaustufe 1 ist umgesetzt: automatischer `Client/resmenu`-Katalog, Suche, TGA/DDS- sowie PNG/JPG/BMP-Vorschau (WIC unter Windows) und read-only NIF-/Materialanalyse für UI-NIFs. Schreibende Interface-Bearbeitung und schreibendes NIF-Material-Editing bleiben offen; AI Workspace ist umgesetzt.
