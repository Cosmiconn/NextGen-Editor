...
- Runtime-Semantik-QA: `UiIconAssets::Texture(...)` unterscheidet jetzt unbekannte IDs von bekannten IDs ohne Raster. Unbekannte semantische IDs werden einmalig geloggt; ein fehlender freigegebener Runtime-Raster bleibt dagegen erwarteter DrawList-Fallback.
...
- QA/Politur: Quest Flow und Custom-NPC/Mob-Vorschau verwenden nun ebenfalls `DrawPanelHeader(...)` mit `module.quest` bzw. `module.custom_npc`/`module.custom_mob`. Reine Entity-Namen, Tabellenzähler und Spezialpicker bleiben bewusst kompakte Inline-Titel.
- DE/EN-QA: Project Hub, Projekt-Konfiguration, Map Launcher und der Topbar-Unsaved-Tooltip enthalten keine fest verdrahteten deutschen sichtbaren UI-Texte mehr. Workspace-Karten, Pfad-Hilfen, Map-Erstellung, Rescan-/Fehlertexte, Recent-Tooltips und Dirty-Details folgen jetzt der globalen Sprachumschaltung.
...
- NIF-Property-Vererbung: `NiAVObject`/`NiNode`-Properties werden child-first entlang der Parent-Kette auf Geometrie aufgelöst; direkte Mesh-Properties überschreiben gleichartige Parent-States. Material, Texturing, Alpha, ZBuffer, Stencil/FaceDrawMode und Specular folgen damit derselben Scene-Graph-Semantik statt nur direkten `NiTriShape`/`NiTriStrips`-Refs. Der NIF-Inspector zeigt die Anzahl tatsächlich geerbter Property-Bindings. `NiTextureEffect` bleibt davon getrennt als noch nicht vollständig materialisierte Semantik offen.

- NIF-Vertexfarben: `NiVertexColorProperty` wird nicht mehr nur übersprungen, sondern als Flags + VertexMode + LightMode erhalten, über dieselbe child-first Property-Kette auf das Mesh aufgelöst und im Renderer als `SRC_IGNORE`, `SRC_EMISSIVE` oder `SRC_AMB_DIF` umgesetzt. Ohne explizite Property bleibt der klassische NIF-Default Ambient+Diffuse bei vorhandenen Vertexfarben erhalten; `VCAlphaTextureBlender` behält seinen separaten belegten Shaderpfad. Der NIF-Inspector zeigt Blockanzahl und effektiven Modus pro Mesh.
...
- Fixture-Materialaudit: 101/101 NIFs laden, 413 Embedded-Texturen werden dekodiert, 0 bleiben undecodiert. Die benannten Shader-Fixtures `FxSkinningBaseMap` (3 Parts in `EglackMad.nif`/`M_MajesticLion.nif`) und `NsPgToonNoAni` (44 Parts in `Female_Hat_Antler00.nif`/`KingdomC00.nif`/`Male_Hat_Antler00.nif`) enthalten im aktuellen Fixture-Korpus **keine ShaderTexDesc-Einträge**. Die Kreuztabelle belegt für beide ausschließlich klassischen Slot 0 + `APPLY_MODULATE`: `FxSkinningBaseMap` 3/3 Parts mit eingebettetem Base-Slot, `NsPgToonNoAni` 44/44 Parts mit Base-Slot (26 embedded, 18 external). Damit gibt es in diesen Fixtures keinen Beleg für fehlende zusätzliche Texturlayer; verbleibende Unterschiede wären Shader-/Beleuchtungslogik und bleiben ohne Referenzshader diagnostisch;
