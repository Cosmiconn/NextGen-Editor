...
- Drop-Table-Ausbaustufe 1 ist umgesetzt: der ShineText-Parser normalisiert das reale `ItemDropTable.txt`-Schema mit 290 Datenspalten plus trailing `;`-Sentinel formatgetreu und besitzt dafür einen >270-Spalten-Roundtrip-/Editier-Regressionstest. Im Spieldaten-Workspace gibt es eine semantische read-only `ItemGroup`-Ansicht mit Mob-/Map-Suche, Basiswerten, 45 Drop-Slots, Rate/Anzahl/Upgrade/Rule, ItemInfo-Auflösung und Ausschlussitems. Schreibende Drop-Bearbeitung folgt erst nach grünem Real-/CI-Roundtrip.
