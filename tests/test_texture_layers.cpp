...
void TestBmpRawRowOrder() {
    // Konvention seit [0.44.27]: BMP-Dateizeile y == Gitterzeile z (Rohreihenfolge, KEIN
    // Bottom-up-Umdrehen) - empirisch an der Karte Rou belegt (Blend-Layer und Vertex-Color-Bitmap
    // passen nur so zur Heightmap-Zeilenachse).
    using namespace theseed::mapeditor::core;
    BlendMap blend(4, 3, 0.0f);
    blend.Set(1, 0, 1.0f); // Gitterzeile 0, Spalte 1
    const auto path = std::filesystem::temp_directory_path() / "nextgen_bmp_raworder_test.bmp";
    Check(legacy::WriteBlendMapBmp(blend, path).has_value(), "BMP schreiben");
    std::ifstream in(path, std::ios::binary);
    std::vector<unsigned char> bytes((std::istreambuf_iterator<char>(in)), {});
    const std::size_t rowSize = 12; // 4 Pixel * 3 Byte, schon auf 4 Byte ausgerichtet
    Check(bytes.size() >= 54 + rowSize * 3, "BMP-Groesse plausibel");
    Check(bytes[54 + 1 * 3] == 255, "Gitterzeile 0 steht als ERSTE Zeile in der Datei (Rohreihenfolge)");
    auto back = legacy::ReadBlendMapBmp(path);
    Check(back.has_value() && back->At(1, 0) > 0.99f && back->At(1, 2) < 0.01f, "Rueckweg liefert dieselbe Zeilenreihenfolge");
}
