...
vec3 SampleLayer(sampler2D diffuseTex, sampler2D blendTex, float uvScale, vec2 mapUv) {
...
void main() {
    ...
    if (uUseTextures && uLayerCount > 0) {
        ...
        vec2 mapUv = vWorldPos.xz / uMapSpan;

        vec3 color = vec3(0.0);
        if (uLayerCount > 0) color += SampleLayer(uDiffuse0, uBlend0, uUvScale[0], mapUv);
        if (uLayerCount > 1) color += SampleLayer(uDiffuse1, uBlend1, uUvScale[1], mapUv);
        if (uLayerCount > 2) color += SampleLayer(uDiffuse2, uBlend2, uUvScale[2], mapUv);
        if (uLayerCount > 3) color += SampleLayer(uDiffuse3, uBlend3, uUvScale[3], mapUv);
        if (uLayerCount > 4) color += SampleLayer(uDiffuse4, uBlend4, uUvScale[4], mapUv);
        if (uLayerCount > 5) color += SampleLayer(uDiffuse5, uBlend5, uUvScale[5], mapUv);
        if (uLayerCount > 6) color += SampleLayer(uDiffuse6, uBlend6, uUvScale[6], mapUv);
        if (uLayerCount > 7) color += SampleLayer(uDiffuse7, uBlend7, uUvScale[7], mapUv);
        baseColor = color;
    } else {
...
void HeightmapRenderer::BeginScene(const OrbitCamera& camera, int width, int height, bool wireframe) {
    ...
    if (indexCount_ > 0) {
        ...
        const Mat4 proj = OrbitCamera::PerspectiveMatrix(
            0.9f, static_cast<float>(width) / static_cast<float>(height), 10.0f, 50000.0f);
...
void HeightmapRenderer::DrawTerrainMesh(const Mat4& viewProj, bool wireframe) {
    ...
    glUniform1i(glGetUniformLocation(shaderProgram_, "uLayerCount"), textureLayerCount_);
    glUniform2f(glGetUniformLocation(shaderProgram_, "uMapSpan"), mapSpanX_, mapSpanZ_);
    glUniform1fv(glGetUniformLocation(shaderProgram_, "uUvScale"), kMaxTextureLayers, layerUvScale_);

    for (int i = 0; i < textureLayerCount_; ++i) {
        glActiveTexture(GL_TEXTURE0 + static_cast<GLenum>(i * 2));
        glBindTexture(GL_TEXTURE_2D, diffuseTex_[i]);
        glUniform1i(glGetUniformLocation(shaderProgram_, ("uDiffuse" + std::to_string(i)).c_str()), i * 2);

        glActiveTexture(GL_TEXTURE0 + static_cast<GLenum>(i * 2 + 1));
        glBindTexture(GL_TEXTURE_2D, blendTex_[i]);
        glUniform1i(glGetUniformLocation(shaderProgram_, ("uBlend" + std::to_string(i)).c_str()), i * 2 + 1);
    }
...
void HeightmapRenderer::DrawTopDownOverlay(std::uint32_t overlayTexture, float alpha) {
