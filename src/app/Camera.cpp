...
void OrbitCamera::OrbitBy(float deltaYawRad, float deltaPitchRad) {
    yaw_ += deltaYawRad;
    pitch_ = std::clamp(pitch_ + deltaPitchRad, MinPitch(), kMaxPitch);
}

void OrbitCamera::Zoom(float deltaDistance) {
    distance_ = std::clamp(distance_ + deltaDistance, kMinDistance, kMaxDistance);
    pitch_ = std::clamp(pitch_, MinPitch(), kMaxPitch);
}

void OrbitCamera::ZoomSteps(float steps) {
    distance_ = std::clamp(distance_ * std::pow(0.88f, steps), kMinDistance, kMaxDistance);
    pitch_ = std::clamp(pitch_, MinPitch(), kMaxPitch);
}

void OrbitCamera::LookBy(float deltaYawRad, float deltaPitchRad) {
    // Augenposition merken, Winkel aendern, Ziel so setzen, dass das Auge stehen bleibt.
    const Vec3 eye{
    ...
    yaw_ += deltaYawRad;
    pitch_ = std::clamp(pitch_ + deltaPitchRad, -1.5f, kMaxPitch);
    targetX_ = eye.x - distance_ * std::cos(pitch_) * std::sin(yaw_);
    targetY_ = eye.y - distance_ * std::sin(pitch_);
    targetZ_ = eye.z - distance_ * std::cos(pitch_) * std::cos(yaw_);
}

void OrbitCamera::MoveLocal(float forward, float right, float up) {
    // Horizontale Blickrichtung im (gespiegelten) Anzeigeraum: f = -(sin(yaw), cos(yaw)).
    const float fx = -std::sin(yaw_), fz = -std::cos(yaw_);
    const float rx = -fz, rz = fx; // = cross(f, up)
    targetX_ += fx * forward + rx * right;
    targetZ_ += fz * forward + rz * right;
    targetY_ += up;
}
...
Mat4 OrbitCamera::ViewMatrix() const {
    ...
    // SPIEGELUNG (CHANGELOG [0.44.27]): Die Editor-Achsen entstehen aus dem Legacy-Z-up-Format
    // durch reines Vertauschen (x,y,z)->(x,z,y) - das ist eine Spiegelung (det -1), keine
    // Drehung. Ohne Gegenmassnahme sieht die ganze Karte spiegelverkehrt aus (z.B. steht, was im
    // Spiel links liegt, hier rechts). Deshalb wird die Welt vor der Kamera an der Z-Achse
    // gespiegelt (Z -> -Z): Kamera-Ziel, Yaw/Pitch und Pan arbeiten im gespiegelten Anzeigeraum,
    // die Welt selbst (Heightmap, Objekte, Server-Koordinaten) bleibt unveraendert. Kein
    // Backface-Culling im Einsatz, daher kein Winding-Problem.
    // BESTAETIGT (David, 22.09.2026): Fiesta rendert direkt mit DirectX 9 (linkshaendiges
    // Koordinatensystem) - passt zu genau diesem Bild: ein Achsentausch aus einem linkshaendigen
    // Quellsystem in unseren (rechtshaendigen, OpenGL-) Renderer ergibt zwangslaeufig eine
    // Spiegelung, keine reine Drehung. Erklaert im Nachhinein auch das in [0.44.34] empirisch
    // gefundene NPC-Blickrichtungs-Vorzeichen (sign=+1 statt der zunaechst angenommenen
    // Spiegelung) - beides folgt aus derselben Haendigkeit, unabhaengig ermittelt, konsistent.
    Mat4 mirrorZ = Mat4::Identity();
    mirrorZ.m[10] = -1.0f;
    return view * mirrorZ;
}
