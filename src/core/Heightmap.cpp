...
Heightmap::Heightmap(std::uint32_t width, std::uint32_t height, float blockWidth, float blockHeight)
    ...
    SetBlockSize(blockWidth, blockHeight); // sanitisiert (siehe Heightmap.hpp): 0/NaN wird ignoriert
}
...
float Heightmap::SampleWorld(float worldX, float worldZ) const noexcept {
    ...
    // NaN-Eingaben (z.B. aus einer 0/0-Division beim Aufrufer) durch std::clamp unveraendert
    // durchgereicht wuerden (NaN erfuellt weder v<lo noch hi<v) und danach als uint32_t UB
    // ausloesen - deshalb hier explizit abfangen, statt sich auf den Aufrufer zu verlassen.
    if (!std::isfinite(worldX)) worldX = 0.0f;
    if (!std::isfinite(worldZ)) worldZ = 0.0f;
