...
TexturePaintPatch PaintLayerWeight(
    ...
    if (layerCount == 0 || targetLayer >= layerCount || settings.radius <= 0.0f) {
        return patch;
    }
    ...
    for (std::int64_t z = std::max<std::int64_t>(0, z0); z <= std::min<std::int64_t>(h - 1, z1); ++z) {
        for (std::int64_t x = std::max<std::int64_t>(0, x0); x <= std::min<std::int64_t>(w - 1, x1); ++x) {
            ...
            // Alte Gewichte aller Layer sichern (für Undo UND Normalisierungs-Basis).
            std::vector<float> oldWeights(layerCount);
            for (std::size_t i = 0; i < layerCount; ++i) {
                oldWeights[i] = stack.Layer(i).blend.At(ux, uz);
            }
            const auto takesPart = [&](std::size_t i) {
...
bool TexturePaintUndoStack::Undo(TextureLayerStack& stack) {
    ...
    TexturePaintPatch patch = std::move(undo_.back());
    undo_.pop_back();

    TexturePaintPatch redoPatch;
...
void TexturePaintUndoStack::Clear() {
    undo_.clear();
    redo_.clear();
}
