...
std::expected<TextureLayerStack, std::string> LoadTsTex(const std::filesystem::path& file) {
    ...
    for (std::uint32_t i = 0; i < layerCount; ++i) {
        ...
        for (std::size_t p = 0; p < pixelCount; ++p) data[p] = static_cast<float>(raw[p]) / 255.0f;
    }
    return stack;
}
...
std::expected<void, std::string> SaveTsTex(const TextureLayerStack& stack, const std::filesystem::path& file) {
    ...
    if (!out) return std::unexpected("Konnte Datei nicht zum Schreiben öffnen: " + file.string());
    ...
    if (!out) return std::unexpected("Fehler beim Schreiben: " + file.string());
    return {};
}
