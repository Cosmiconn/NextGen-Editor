...
std::expected<ShnFile, std::string> LoadShnFile(const std::filesystem::path& path) {
    ...
    for (std::uint32_t i = 0; i < *colCount; ++i) {
        ...
        // Nur ECHT leere Namen bekommen einen Platzhalter. Frueher galt das schon fuer Namen mit
        // < 2 Zeichen - dadurch wurden die echten Spalten "X"/"Y" (z.B. TownPortal.shn) beim
        // Laden zu "Undefined 4/5" umbenannt und beim Speichern so in die Datei geschrieben.
        if (c.name.empty()) { c.name = "Undefined " + std::to_string(i); c.synthesizedName = true; }
...
std::expected<void, std::string> SaveShnFile(const ShnFile& file, const std::filesystem::path& path) {
    ...
    for (const auto& c : file.columns) {
        w.WritePaddedString(c.synthesizedName ? std::string() : c.name, 48);
        w.Write(c.type);
        w.Write(c.length);
    }
