...
std::optional<std::filesystem::path> ResolveLegacyAssetPath(
    ...
    if (!assetRoot.empty()) {
        ...
        // Robust-Fallback für ältere/inkonsistente NIF-Pfade: Wenn der Pfad zwar
        // existiert, aber die Ablage unter einem anderen resmap-Unterordner liegt,
        // suche anhand des Dateinamens. Mehrdeutige Treffer werden weiterhin verworfen.
        // Das verhindert, dass gültige Modelle nur wegen eines abweichenden Präfixes
        // (z.B. resmap\nif vs. nif) komplett unsichtbar bleiben.
        if (stripped.has_filename()) {
            const std::string filename = stripped.filename().string();
            for (const auto& root : {mapDir, assetRoot}) {
                if (auto fallback = FindFileRecursiveBounded(root, filename, -1)) {
                    return fallback;
                }
            }
        }
        ...
        if (std::distance(stripped.begin(), stripped.end()) == 1) {
            ...
            for (const char* sharedFolder : {"fieldTexture", "FieldTexture", "fieldtexture"}) {
                ...
                if (auto nested = FindFileRecursiveBounded(candidateDir, bareName, -1)) {
                    return nested;
                }
