...
std::expected<void, std::string> WriteBlendMapBmp(const BlendMap& blend, const std::filesystem::path& file) {
    ...
    // --- Pixeldaten: Rohreihenfolge (Zeile y = Gitterzeile z), je Pixel B,G,R, auf 4 Byte aufgef\u00fcllt ---
    std::vector<std::uint8_t> row(rowSize, 0);
    for (std::uint32_t y = 0; y < height; ++y) {
        // Dateizeile y == Gitterzeile z (Rohreihenfolge, KEIN Bottom-up-Umdrehen) - siehe
        // ReadBlendMapBmp und CHANGELOG [0.44.27].
        const std::uint32_t z = y;
...
std::expected<BlendMap, std::string> ReadBlendMapBmp(const std::filesystem::path& file) {
    ...
    const auto h = static_cast<std::uint32_t>(height < 0 ? -height : height);
    ...
    for (std::uint32_t y = 0; y < h; ++y) {
        ...
        // WICHTIG (CHANGELOG [0.44.27]): Dateizeile y == Gitterzeile z, die BMP-Zeilen werden in
        // ROH-Reihenfolge gelesen und NICHT (bottom-up) umgedreht. Empirisch an der echten Karte
        // "Rou" belegt: nur so decken sich die Blend-Layer (Korrelation "rock" mit der Hoehe
        // +0.79 statt -0.06) und die Vertex-Color-Bitmap mit der Heightmap, deren Zeilenachse
        // wiederum mit Objekt-Positionen (Median-Hoehenfehler 4.9 statt >=129), dem Walk-Grid und
        // den Server-Koordinaten (NPC.txt/MapWayPoint.shn) uebereinstimmt. Das Spiel liest die
        // Bitmaps also offenbar ohne Bottom-up-Umkehr - das frueher hier stehende Umdrehen war die
        // Ursache der "Textur oben/unten vertauscht"-Meldungen.
        const std::uint32_t z = y;
